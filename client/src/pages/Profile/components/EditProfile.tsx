...
const EditProfile = () => {
  ...
  useEffect(() => {
    if (userProfile?.data) {
      reset({
        ...
        // profession:userProfile?.data?.data?.interests[0] || "",
        profilePhoto: userProfile?.data?.data?.profilePic|| "",
      });
      if (!selectedImage && userProfile?.data?.data?.profilePic) {
        setSelectedImage(
          `data:image/jpeg;base64,${userProfile?.data?.data.profilePic}`
        );
      }
    }
  }, [userProfile?.data]);
  ...
  const onSubmit = (data) => {
    ...
    // if (dirtyFields.profession) {
    //   updatedFields.interests = userProfile?.data?.data?.interests
    //     ? [...userProfile.data.data.interests, data.profession]
    //     : [data.profession];
    // }

    if (
      selectedImage &&
      selectedImage.split(",")[1] !== userProfile?.data?.data?.profilePic
    ) {
      updatedFields.profilePic = selectedImage.split(",")[1];
    }
  ...
  return (
    <Card className="flex flex-col relative gap-0 justify-center items-center min-h-[80vh] bg-muted/10 px-4 py-8 mt-4">
      <Link
        to="/profile"
          ...
          <label htmlFor="file" className="cursor-pointer relative group">
            <Avatar className="w-44 h-44 rounded-full shadow border-4 border-primary">
              <AvatarImage
                src={
                  selectedImage ||
                  (userProfile?.data?.data?.profilePic
                    ? `data:image/jpeg;base64,${userProfile?.data?.data?.profilePic}`
                    : "")
                }
                alt="user img"
          ...
          {/* <FormField
            ...
            render={({ field }) => (
              <FormItem className="w-full">
                ...
                <FormControl>
                  <Input {...field} placeholder="Enter your Area of Interest" />
                 
                </FormControl>
                <div className="text-xs text-gray-500">(Coding,Drawing,Playing) only allowed</div>
                <FormMessage />
              </FormItem>
            )}
          /> */}
