...
import UserCard from "./components/FriendsCard";
...
const Friends = () => {
  ...
  const [isError, setIsError] = useState(false); // Simulate error state
  ...
  useEffect(()=>{
    const fetchUser=async()=>{
     setIsLoading(true)
     try {
       const res=await api.get('/user/friends-list');
       console.log(res.data)
       if(res.data.success){
         setIsLoading(false)
         setUsers(res.data.data)
         toast.success("Friends list fetched successfully")
       }
     } catch (error) {
       setIsLoading(false);
       setIsError(true)
       toast.error("Failed to fetch friends list list")
     }
    }
    fetchUser()
   },[])
